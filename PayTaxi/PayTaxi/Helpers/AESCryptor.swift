...
/// The `AESCryptorType` protocol defines generic API to a mutable,
...
public protocol AESCryptorType {
...
public extension AESCryptorType {
    /// Simplified, generic interface to `AESCryptorType`. Takes a data,
...
/// AESCryptor encryption/decryption interface.
public enum AESCryptor {
    
    /// Errors thrown by `AESCryptorType`.
    ...
    /// A encryptor for the latest data format. If compatibility with other AESCryptor
    /// implementations is required, you may wish to use the specific encryptor version rather
    /// than accepting "latest."
    ///
    public final class Encryptor: AESCryptorType {
    ...
    public final class Decryptor : AESCryptorType {
        private var decryptors: [VersionedDecryptorType.Type] = [DecryptorV3.self]
        
        private var buffer = Data()
        private var decryptor: AESCryptorType?
...
public extension AESCryptor {
    ...
    public final class EncryptorV3 : AESCryptorType {
        ...
        public convenience init(password: String) {
            self.init(
                password: password,
                encryptionSalt: AESCryptor.randomData(ofLength: V3.saltSize),
                hmacSalt: AESCryptor.randomData(ofLength: V3.saltSize),
                iv: AESCryptor.randomData(ofLength: V3.ivSize))
        }
        ...
        public convenience init(encryptionKey: Data, hmacKey: Data) {
            self.init(encryptionKey: encryptionKey, hmacKey: hmacKey, iv: AESCryptor.randomData(ofLength: V3.ivSize))
        }
...
internal final class Engine {
    ...
    func update(withData data: Data) -> Data {
        ...
        assert(result == CCCryptorStatus(kCCSuccess), "AESCRYPTOR BUG. PLEASE REPORT. (\(result)")
...
internal typealias V3 = AESCryptor.FormatV3
...
private final class DecryptorEngineV3 {
    ...
    func finalData() throws -> Data {
        ...
        if !isEqualInConsistentTime(trusted: hash, untrusted: buffer.finalData()) {
            throw AESCryptor.Error.hmacMismatch
        }
...
private protocol VersionedDecryptorType: AESCryptorType {
